...
impl Shader {
    ...
    pub fn new(vertex_path: &str, fragment_path: &str) -> Shader {
        ...
        // vertex
        let mut vertex_file = File::open(vertex_path).unwrap_or_else(|_| panic!("failed to open file: {}", vertex_path));

        let mut fragment_file = File::open(fragment_path).unwrap_or_else(|_| panic!("failed to open file: {}", fragment_path));

        let mut vertex_code = String::new();
        
        // fragment
        let mut fragment_code = String::new();
        vertex_file.read_to_string(&mut vertex_code).expect("failed to read vertex shader file");
        fragment_file.read_to_string(&mut fragment_code).expect("failed to read fragment shader file");
    ...
    pub fn with_geometry_shader(
        ...
        let mut vertex_file = File::open(vertex_path).unwrap_or_else(|_| panic!("failed to open file: {}", vertex_path));
        let mut fragment_file = File::open(fragment_path).unwrap_or_else(|_| panic!("failed to open file: {}", fragment_path));
        let mut geometry_file = File::open(geometry_path).unwrap_or_else(|_| panic!("failed to open file: {}", geometry_path));

        let mut vertex_code = String::new();
        let mut fragment_code = String::new();
        let mut geometry_code = String::new();
        vertex_file.read_to_string(&mut vertex_code).expect("failed to read vertex shader");
        fragment_file.read_to_string(&mut fragment_code).expect("failed to read fragment shader");
        geometry_file.read_to_string(&mut geometry_code).expect("failed to read geometry shader");
